...
public sealed class BwRepositoryParser
{
    ...
    private static SourceLocation Line(string path, XElement node) { var li = (IXmlLineInfo)node; return new SourceLocation { FilePath = path, Line = li.HasLineInfo() ? li.LineNumber : null, Column = li.HasLineInfo() ? li.LinePosition : null }; }
